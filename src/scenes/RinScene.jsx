...
import { Sparkles, Gem, Star, Wand2, Diamond } from 'lucide-react'
import rinImg from '../assets/rin.jpg'
import '../App.css'

// Deterministic ember/spark particles — larger & brighter
const SPARKS = Array.from({ length: 110 }, (_, i) => ({
  cx: Math.round((i * 137.508) % 1920),
  cy: Math.round((i * 97.333) % 1080),
  r: +(1.8 + (i % 5) * 1.10).toFixed(1),
  opacity: +(0.40 + (i % 8) * 0.07).toFixed(2),
}))

// Floating Lucide icon positions for the magic atmosphere
const RIN_ICONS = [
  { Icon: Sparkles, x: '7%',  y: '16%', size: 34, color: '#ffe066', dur: '3.2s' },
  { Icon: Gem,      x: '13%', y: '34%', size: 28, color: '#ff99dd', dur: '4.0s' },
  { Icon: Wand2,    x: '18%', y: '55%', size: 24, color: '#ffcc88', dur: '2.9s' },
  { Icon: Star,     x: '46%', y: '8%',  size: 22, color: '#ffeebb', dur: '3.7s' },
  { Icon: Sparkles, x: '68%', y: '7%',  size: 26, color: '#ffdd55', dur: '2.6s' },
  { Icon: Diamond,  x: '78%', y: '14%', size: 22, color: '#ffbbee', dur: '3.4s' },
  { Icon: Gem,      x: '84%', y: '25%', size: 20, color: '#ffaaee', dur: '2.4s' },
  { Icon: Star,     x: '88%', y: '58%', size: 18, color: '#ffddaa', dur: '4.2s' },
  { Icon: Sparkles, x: '72%', y: '72%', size: 20, color: '#ffcc44', dur: '3.0s' },
  { Icon: Wand2,    x: '28%', y: '78%', size: 18, color: '#ffaa55', dur: '2.7s' },
  { Icon: Diamond,  x: '55%', y: '82%', size: 16, color: '#ffccff', dur: '4.5s' },
]
...
export default function RinScene() {
  ...
  return (
    <div className="page">
      <ul ref={sceneRef} className="scene">
          ...
          <svg className="lsvg" viewBox="0 0 1920 1080" preserveAspectRatio="xMidYMid slice" aria-hidden="true">
            {SPARKS.map((s, i) => (
              <circle
                ...
                cy={s.cy}
                r={s.r}
                fill={i % 3 === 0 ? '#ffe066' : i % 3 === 1 ? '#ffaa44' : '#ff7733'}
                opacity={s.opacity}
                style={{
        ...
        {/* Layer 2b — Floating Lucide magic icons */}
        <li data-depth="0.35" className="layer">
          <div className="icon-scatter">
            {RIN_ICONS.map((ic, i) => (
              <span
                key={i}
                className="icon-float"
                style={{ left: ic.x, top: ic.y, '--dur': ic.dur, '--delay': `${(i * 0.38).toFixed(2)}s`, '--glow': ic.color }}
              >
